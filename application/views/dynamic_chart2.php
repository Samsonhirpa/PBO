...
function load_monthwise_data(year, title)
{
    ...
    $.ajax({
        url:"<?php echo base_url(); ?>dynamic_chart/fetch_data2",
        method:"POST",
        data:{year:year},
        dataType:"JSON",
...
function drawMonthwiseChart(chart_data, chart_main_title)
{
    ...
    data.addColumn('number', 'totchild');

    $.each(jsonData, function(i, jsonData){
        var month = jsonData.month;
        var totchild = parseFloat($.trim(jsonData.totchild));
        data.addRows([[month, totchild]]);
    });
...
$(document).ready(function(){
    $('#year').change(function(){
        ...
        if(year != '')
        {
            load_monthwise_data(year, 'Waliigala Daayimmanii Kan Bara/Waggaa');
        }
    });
});
